{{-- MODAL CREATE NEW Phonebook --}}
@if (Auth::user()->hasPermissionTo('pbxadmin.phonebook.store'))
	<x-modal id="modal_create_phonebook">
		<x-slot:title>
			Create Phonebook
		</x-slot:title>

		<x-slot:content>
			<form action="{{route('pbxadmin.phonebooks.store')}}" method="POST" id="modal-form" enctype="multipart/form-data">
				@csrf

				<div class="fv-row mb-10">
					<label class="d-flex align-items-center fs-5 fw-bold mb-2">
						<span class="required">Phonebook Name</span>
						<x-tooltip title="Required: The name of the Phonebook to create."/>
					</label>
					...
					<select class="form-select" aria-label="Select Site" name="site_id" id="site-select">
						@foreach($sites as $site)
							<option value="{{$site->id}}"
									@if($site->id == $currentSiteId) selected @endif>{{$site->name}}</option>
						@endforeach
					</select>
				</div>
				<div class="mb-10">
					...
					<br>
					<a href="{{asset('assets/samples/sample_bulk_uploads_no_ext.csv')}}" download="phonebooks_bulk_upload.csv">Download the Sample CSV</a>
				</div>
			</form>
		</x-slot:content>
		...
		<x-slot:footer>
			...
			<x-jet-button type="button" class="px-6" form="modal-form" id="submit-button-create-phonebook"
