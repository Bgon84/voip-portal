...
@if (Auth::user()->hasPermissionTo('pbxadmin.album.store'))
	<x-modal id="modal_create_webhook">
		<x-slot:title>
			Create Album
		</x-slot:title>

		<x-slot:content>
			<form action="{{route('pbxadmin.albums.store')}}" method="POST" id="modal-form" enctype="multipart/form-data">
				@csrf

				<div class="fv-row mb-10">
					<label class="d-flex align-items-center fs-5 fw-bold mb-2" for="name">
						<span class="required">Album Name</span>
						<x-tooltip title="Required: name of the Album to create."/>
					</label>
				...
				<div class="mb-10">
					...
					<br>
					<a href="{{asset('assets/samples/sample_bulk_albums.csv')}}" download="albums_bulk_upload.csv">Download the Sample CSV</a>
				</div>
